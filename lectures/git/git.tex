...
\begin{frame}
    ...
    \begin{itemize}
        \ii{Git använder termen \emph{remote}. Lokal ``spegling'' av fjärrrepository.}
...
\begin{frame}
    \frametitle{Båda samtidigt!? -- push (TODO!)}

    % \begin{center}
    %         \includegraphics[width=.9\textwidth]{figs/local_remote_commit_fetch.png}
    % \end{center}

\end{frame}

\begin{frame}
    \frametitle{Båda samtidigt!? -- fetch (TODO!)}
...
\begin{frame}
    \frametitle{Lokala och fjärrändringar i Git}

    \begin{block}{Vad händer om både lokala och fjärrändringar finns?}
        \begin{itemize}
            \ii{\emph{git fetch} hämtar fjärrändringar, uppdaterar din lokala spegling av remote, men påverkar inte dina lokala grenar.}
            \ii{\emph{git pull} försöker automatiskt slå samman de inkommande  ändringarna med dina lokala grenar.}
            \ii{Om konflikt uppstår vid merge måste du lösa den manuellt.}
            \ii{Efter att konflikter är lösta, kan du \emph{commit} och \emph{push} dina ändringar.}
        \end{itemize}
    \end{block}

    \begin{block}{``Ocommittade'' ändringar}
        \begin{itemize}
            \ii{Om du har lokala ändringar (i ditt workspace) som inte är committade, kan du göra merge.}
            \ii{Lösning: gör först \emph{commit} eller \emph{stash}.}
        \end{itemize}
    \end{block}
