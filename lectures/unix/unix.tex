...
\begin{frame}[fragile=singleslide]
    ...
    Om \code{\url{~}/dod/lab1} är aktuell katalog:
...
\begin{frame}[fragile=singleslide]
    ...
    Exempel på shells:
    \begin{itemize}
        \item \code{bash} (Bourne-Again SHell)
        \item \code{sh}, \code{zsh}, \code{csh}
    \end{itemize}
...
\begin{frame}[fragile=singleslide]
    ...
    \begin{itemize}
        \item Öppna Terminal och skriv \code{bash} för att temporärt byta till Bash.
        \item Installera senaste Bash via Homebrew: \code{brew install bash}
    \end{itemize}
...
\begin{frame}[fragile=singleslide]
    ...
    \textbf{Filägande:}
    \begin{itemize}
        \item Ägare identifieras via användarnamn och grupp (t.ex., \code{ma1337no-s} i \code{students})
    \end{itemize}
...
\begin{frame}[fragile=singleslide]
    ...
    \textbf{\code{bash} initieringsfil:}
    \begin{itemize}
        \item bash läser \code{.bash\_profile} (från hemkatalogen) när ett nytt terminalfönster öppnas.
...
\begin{frame}[fragile=singleslide]
    ...
    \begin{description}
        \item[Inbyggda] Direkt exekverade i kommandotolken (t.ex. \code{cd}, \code{pwd}, \code{exit}).
        \item[Vanliga program] Kommandotolken söker efter programfiler i \code{PATH}.
    \end{description}

    \textbf{Programexekvering:}
    \begin{itemize}
        \item \code{PATH} definierar var program letas upp (\code{export PATH=...} för anpassning).
        \item Ex: \code{/usr/bin/javac} finns i \code{PATH}, så \code{javac X.java} går att köra.
    \end{itemize}
    ...
    \begin{Code}
        ./programnamn
    \end{Code}
    Använd \code{./} för program i den aktuella katalogen, eftersom \code{.} inte ingår i \code{PATH}.
...
\begin{frame}[fragile=singleslide]
    ...
    \begin{Code}
        gedit example.txt &
    \end{Code}
    \textit{Öppnar \code{gedit} i bakgrunden, terminalen blir omedelbart tillgänglig för nya kommandon.}
